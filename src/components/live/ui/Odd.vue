...
<!-- 完全沒資料 -->
    ...
    <div v-else class="spread-odd">
      <div class="spread">
        <div>{{ gameData.score1 }} </div>
        <div>{{ getHA(gameData.odds) }}</div>
      </div>
      <!-- <div></div> -->
      <div class="spread">
        <div>{{ gameData.score2 }} </div>
        <div>{{ getOU(gameData.odds) }}</div>
      </div>
    </div>
<!-- HA為負 -->
...
import { ISiteDto, ISiteOdd } from '@/type/LiveChat'
export default defineComponent({
  props: {
    data: {} as PropType<ISiteDto>
  },
  setup(props) {
    const gameData = toRefs(props).data as Ref<ISiteDto>

    const getHA = (spread: Array<ISiteOdd>) => {
      const odd = spread.filter(x => x.playMode == 'HA')[0]
      if (!odd) return '-'
      const price = odd.prices.filter(x => x.main && x.spread != '1X2')[0]
      if (!price) return '-'
      return price.spread
    } 
    
    const getOU = (spread: Array<ISiteOdd>) => {
      const odd = spread.filter(x => x.playMode == 'OU')[0]
      if (!odd) return '-'
      const price = odd.prices.filter(x => x.main)[0]
      if (!price) return '-'
      return price.spread
    }
